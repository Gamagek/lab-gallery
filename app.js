    ...
    if (galleryGrid) {
        galleryGrid.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 40px; background: var(--card-bg); border-radius: 10px; border: 1px solid var(--border-color);">
                ...
                <p style="margin: 0; color: var(--text-muted);">Retrieving multi-product comparison specs, Cloudflare video streams, rich descriptions, and JSON-LD schema.</p>
            </div>
        `;
    }
...
function renderCards(items) {
    ...
    items.forEach((item) => {
        ...
        let mediaElement = '';
        if (videoUrl) {
            mediaElement = `<figure class="media-figure" style="margin:0;"><video controls preload="metadata" style="width:100%; height:260px; background:#000; object-fit:cover; display:block;"><source src="${videoUrl}" type="video/mp4">Your browser does not support video.</video></figure>`;
        } else if (imageUrl) {
            mediaElement = `<figure class="media-figure" style="margin:0;"><img src="${imageUrl}" alt="${alt}" loading="lazy" style="width:100%; height:260px; object-fit:cover; display:block;"></figure>`;
        }
